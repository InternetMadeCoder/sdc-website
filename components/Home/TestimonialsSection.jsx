...
const TestimonialsSection = () => {
  ...
  return (
    <div className="py-8 px-4 sm:px-6 md:px-12 lg:px-20 xl:px-28 bg-white flex flex-col gap-12 items-start relative z-30 md:items-center white-section">
      ...
      {/* Static testimonials */}
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-8 lg:max-w-7xl place-items-center">
        {staticTestimonials.map((testimonial) => (
          <div
            key={testimonial.id}
            className="relative pt-16 hover:transform hover:scale-105 transition-transform duration-300 w-full"
          >
            ...
            <div className="bg-gradient-to-br from-indigo-950 via-purple-900 to-blue-950 text-white rounded-2xl p-4 pt-14 min-h-[250px] md:h-[250px] flex flex-col items-center shadow-xl hover:shadow-2xl transition-shadow duration-300">
              ...
              <p className="text-sm text-justify px-3">{testimonial.message}</p>
            </div>
          </div>
        ))}
      </div>
